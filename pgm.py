...
def route_login():
  ...
  if '_cas_token' in flask.session:
    if validate(flask.session['_cas_token']):
      redirect_url = flask.url_for('edit_db')
    else:
      redirect_url = create_cas_login_url(WSApp.config['CAS_SERVER'], '/cas/login/')
...
if __name__ == "__main__":
  # @TODO make a more secret key
  WSApp.secret_key = 'very secret key'
  WSApp.config['SESSION_TYPE'] = 'filesystem'
  WSApp.config['CAS_SERVER'] = 'https://astrocas.wr.usgs.gov/'
  WSApp.config['CAS_LOGIN_ROUTE'] = '/cas/login/'
  WSApp.config['CAS_AFTER_LOGIN'] = 'indexHandler'
  cas.init_app(WSApp)
